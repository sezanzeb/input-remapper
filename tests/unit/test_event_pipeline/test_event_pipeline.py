...
class TestIdk(EventPipelineTestBase):
    ...
    async def test_can_not_map(self):
        ...
        mapping_1 = Mapping(
            event_combination=InputCombination(get_combination_config(ev_2)),
            target_uinput="keyboard",
            output_type=EV_KEY,
            output_code=BTN_TL,
        )
        mapping_2 = Mapping(
            event_combination=InputCombination(get_combination_config(ev_3)),
            target_uinput="keyboard",
            output_type=EV_KEY,
            output_code=KEY_A,
        )
    ...
    async def test_axis_switch(self):
        ...
        cfg = {
            "event_combination": combination.to_config(),
            "target_uinput": "mouse",
            "output_type": EV_REL,
            "output_code": REL_X,
            "rel_rate": rel_rate,
