...
class TestBackend(unittest.TestCase):
    ...
    def test_should_provide_groups(self):
        data_bus = DataBus()
        groups = _Groups()
        backend = get_backend(reader=Reader(data_bus, groups))
        self.assertIs(backend.groups, groups)

    def test_should_emit_groups(self):
        data_bus = DataBus()
        backend = get_backend(data_bus=data_bus, reader=Reader(data_bus, _Groups()))
        listener = Listener()
        data_bus.subscribe(MessageType.groups, listener)
    ...
    def test_should_emit_uinputs(self):
        ...
        backend = get_backend(data_bus=data_bus, reader=Reader(data_bus, _Groups()))
        listener = Listener()
        data_bus.subscribe(MessageType.uinputs, listener)
